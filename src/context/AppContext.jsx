...
import { chargingStations, userFavorites as initialFavorites } from '../data/mockData';
...
export const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (filters.operator !== 'all') {
      filtered = filtered.filter(station => station.operator.toLowerCase() === filters.operator);
    }
  ...
  // Actions
  const actions = {
    setStations: (stations) => dispatch({ type: ActionTypes.SET_STATIONS, payload: stations }),
    setSelectedStation: (station) => dispatch({ type: ActionTypes.SET_SELECTED_STATION, payload: station }),
    setUserLocation: (location) => dispatch({ type: ActionTypes.SET_USER_LOCATION, payload: location }),
    toggleFavorite: (stationId) => dispatch({ type: ActionTypes.TOGGLE_FAVORITE, payload: stationId }),
    setFilters: (filters) => dispatch({ type: ActionTypes.SET_FILTERS, payload: filters }),
    updateFilter: (key, value) => dispatch({ type: ActionTypes.UPDATE_FILTER, payload: { key, value } }),
    setSearchQuery: (query) => dispatch({ type: ActionTypes.SET_SEARCH_QUERY, payload: query }),
    setLoading: (loading) => dispatch({ type: ActionTypes.SET_LOADING, payload: loading }),
    setError: (error) => dispatch({ type: ActionTypes.SET_ERROR, payload: error }),
    setMapCenter: (center) => dispatch({ type: ActionTypes.SET_MAP_CENTER, payload: center }),
    setMapZoom: (zoom) => dispatch({ type: ActionTypes.SET_MAP_ZOOM, payload: zoom }),
